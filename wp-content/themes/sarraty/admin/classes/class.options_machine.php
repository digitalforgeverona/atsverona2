...
class Options_Machine {
	...
	public static function optionsframework_machine($options) {
		...
		foreach ($options as $value) {
			...
			/* condition start */
			if(!empty($smof_data) || !empty($asalah_data)){
			...
			switch ( $value['type'] ) {
				...
				case 'mega':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= Options_Machine::optionsframework_menus_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
						foreach ($slides as $slide) {
							$oldorder = $slide['order'];
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button menu_add_button">Add New Item</a></div>';
				...
				case 'sorter':
				    ...
				    $sortlists = isset($asalah_data[$value['id']]) && !empty($asalah_data[$value['id']]) ? $asalah_data[$value['id']] : $value['std'];
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __('Last Backup : ', 'asalah').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	public static function optionsframework_menus_function($id,$std,$oldorder,$order){
			...
			//initialize all vars
			$slidevars = array('title', 'width', 'description', 'type' );
			...
			$slider .= '<label>Width</label>';
			$slider .= '<input class="slide of-input of-slider-width" name="'. $id .'['.$order.'][width]" id="'. $id .'_'.$order .'_slide_width" value="'. stripslashes($val['width']) .'" />';
			$slider .= '<div id="'. $id .'_'.$order .'_slide_width-slider" class="smof_sliderui ui-slider ui-slider-horizontal ui-widget ui-widget-content ui-corner-all" style="margin-left: 7px;" data-id="'. $id .'_'.$order .'_slide_width" data-val="'.$width_value.'" data-min="50" data-max="930" data-step="1" aria-disabled="false"><div class="ui-slider-range ui-widget-header ui-corner-all ui-slider-range-min" style="width: 0%;"></div><a class="ui-slider-handle ui-state-default ui-corner-all" href="#" style="left: 0%;"></a></div>';
			
			$slider .= '<br /><br /><label>Content</label>';
			$slider .= '<textarea class="slide of-input" name="'. $id .'['.$order.'][description]" id="'. $id .'_'.$order .'_slide_description" cols="8" rows="8">'.stripslashes($val['description']).'</textarea>';
			
			
			$slider .= '<label>Dropdown Opens To</label>';
			$slider .= '<select class="select of-input" name="'. $id .'['.$order.'][type]" id="'. $id .'_'.$order .'_slide_type">';
			$types = array('left' => 'Left', 'right' => 'Right');
			foreach ($types as $key => $value) {			
				$slider .= '<option value="'.$key.'" ' . selected($val['type'], $key, false) . ' />'.$value.'</option>';	 
			} 
			$slider .= '</select>';
