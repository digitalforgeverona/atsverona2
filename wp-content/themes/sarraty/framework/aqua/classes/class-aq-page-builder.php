...
if(!class_exists('AQ_Page_Builder')) {
	class AQ_Page_Builder {
		...
		function __construct( $config = array()) {
			...
			$defaults['menu_title'] = __('Page Builder', 'asalah');
			$defaults['page_title'] = __('Page Builder', 'asalah');
			$defaults['page_slug'] = __('aq-page-builder', 'asalah');
		...
		function admin_enqueue() {
			...
			// Register 'em
			wp_register_style( 'aqpb-css', get_template_directory_uri().'/framework/aqua/assets/css/aqpb.css', array(), time(), 'all');
			wp_register_style( 'aqpb-blocks-css', get_template_directory_uri().'/framework/aqua/assets/css/aqpb_blocks.css', array(), time(), 'all');
			wp_register_script('aqpb-js', get_template_directory_uri().'/framework/aqua/assets/js/aqpb.js', array('jquery'), time(), true);
			wp_register_script('aqpb-fields-js', get_template_directory_uri().'/framework/aqua/assets/js/aqpb-fields.js', array('jquery'), time(), true);
		...
		function view_enqueue() {
			...
			// front-end css
			wp_register_style( 'aqpb-view-css', get_template_directory_uri().'/framework/aqua/assets/css/aqpb-view.css', array(), time(), 'all');
			wp_enqueue_style('aqpb-view-css');
				
			// front-end js
			wp_register_script('aqpb-view-js', get_template_directory_uri().'/framework/aqua/assets/js/aqpb-view.js', array('jquery'), time(), true);
		...
		**/
		function display_template($template_id) {
			...
			if(empty($blocks)) {
				...
				$first = false; $last = false; $missedlast = false;
				...
				foreach($blocks as $key => $instance) {
					...
					if(class_exists($id_base)) {
						...
						if($parent == 0) {
							...
							$last = false; //reset $last
							$missedlast = false; //reset $missedlast
						}
					}
				}
		...
		function add_media_button( $button ) {
			...
			if ( in_array( $pagenow, array( 'post.php', 'page.php', 'post-new.php', 'post-edit.php' ) ) ) {
				...
				if ( version_compare( $wp_version, '3.5', '<' ) ) {
					$img 	= '<img src="' . get_template_directory_uri() . '/framework/aqua/assets/images/aqua-media-button.png" width="16px" height="16px" alt="' . esc_attr__( 'Add Page Template', 'framework' )  . '" />';
					...
					$img 	= '<span class="wp-media-buttons-icon" style="background-image: url(' . get_template_directory_uri() . '/framework/aqua/assets/images/aqua-media-button.png ); margin-top: -1px;"></span>';
		...
		function contextual_help() {
			...
			if($screen->id == $this->page) {
				...
				$screen->set_help_sidebar(
					'<p><strong>' . __('For more information:', 'asalah') . '</strong></p>' .
					'<p>' . __('<a href="http://aquagraphite.com/api/documentation/aqua-page-builder" target="_blank">Documentation</a>', 'asalah') . '</p>' .
					'<p>' . __('<a href="http://aquagraphite.com/api/changelog/aqua-page-builder" target="_blank">Changelog</a>', 'asalah') . '</p>'
				);
				
				// Main overview tab
				$screen->add_help_tab( array(
				'id'		=> 'overview',
				'title'		=> __('Overview', 'asalah'),
