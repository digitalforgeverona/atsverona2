...
(function( $, window, document, undefined ) {
	...
	var Carousel = {
		...
		updateItems : function(){
			...
			var width = $(base.options.responsiveElementWidth).width();
		...
		goTo : function(position,pagination){
			...
			if(base.support3d === true){
				...
				} else if(pagination === "rewind" ){
					base.swapTransitionSpeed(base.options.rewindSpeed);
					setTimeout(function() {
						base.isCss3Finish = true;
		...
		play : function(){
			...
			base.autoPlayInterval = setInterval(function(){
				if(base.options.autoPlayDirection === "ltr"){
					base.next(true);
				} else if(base.options.autoPlayDirection === "rtl"){
					base.prev(true)
				}
		...
		startPosition : function(){
			var base = this;
			if(base.options.startPosition === 0){
				return false;
			}
			if(base.options.startPosition >= 0){
				var position = base.options.startPosition;
			} else if(base.options.startPosition <0){
				var position = base.maximumSlide + base.options.startPosition + 1;
			}
			if(position >= base.maximumSlide){
				base.currentSlide = position = base.maximumSlide;
			} else if(position < -base.maximumSlide){
				base.currentSlide = position = base.maximumSlide;
			} else {
				base.currentSlide = position;
			}
			base.swapTransitionSpeed(0)
			if(base.support3d === true){
				base.transition3d(base.positionsInArray[position]);
			} else {
				base.css2slide(base.positionsInArray[position], 0);
			}
		}
	};
