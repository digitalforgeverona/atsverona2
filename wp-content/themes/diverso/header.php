...
<title><?php
    ...
    <?php
		...
		//if ( ( is_home() || is_front_page() ) ) :
            if( !in_array( $slider_type, array('none','fixed-image')) ) {
				...
				elseif ( $slider_type == 'cycle' ) {
                    wp_enqueue_script( 'jquery-slides', get_template_directory_uri()."/js/jquery.slides.min.js", array('jquery'), '1.1.1' );   
                    wp_enqueue_script( 'swfobject' );
                }
		...
		if ( isset( $post->ID ) ) {
    		$accordion_slider = get_post_meta( get_the_ID(), '_slider_accordion', true );     
