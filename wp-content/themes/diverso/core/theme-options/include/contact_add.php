			...
			<tbody>
				...
				<tr class="options-list toggled<?php if( $attrs['type'] != 'select' AND $attrs['type'] != 'radio' ) : ?> hide-if-js<?php endif; ?>">           
					...
					<td class="field" colspan="2">                                        
						...
						<p class="option">      
							<label><input type="radio" name="<?php yiw_name_contact_field( 'option_selected' ) ?>" value="<?php echo ( $id > 0 ) ? $id + 1 : 0 ?>" /> <?php _e( 'Selected', 'yiw' ) ?></label>
		...
		jQuery(document).ready(function($){   
			...
			$('.media-upload-form').live( 'submit', function(){			                                 
				...
				setTimeout( function() {
					...
					$.ajax({
						url: '<?php echo admin_url( 'themes.php' ) ?>',
						data: datastring,
						type: 'GET',
