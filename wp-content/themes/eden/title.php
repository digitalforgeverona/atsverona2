...
if(get_post_meta($id, "qode_responsive-title-image", true) != ""){
 $responsive_title_image = get_post_meta($id, "qode_responsive-title-image", true);
	...
	$responsive_title_image = $qode_options_eden['responsive_title_image'];
}

if(get_post_meta($id, "qode_fixed-title-image", true) != ""){
 $fixed_title_image = get_post_meta($id, "qode_fixed-title-image", true);
}else{
	$fixed_title_image = $qode_options_eden['fixed_title_image'];
}

if(get_post_meta($id, "qode_title-image", true) != ""){
 $title_image = get_post_meta($id, "qode_title-image", true);
}else{
	$title_image = $qode_options_eden['title_image'];
}
$title_image_height = "";
$title_image_width = "";
...
if(get_post_meta($id, "qode_title-overlay-image", true) != ""){
 $title_overlay_image = get_post_meta($id, "qode_title-overlay-image", true);
	...
	if(isset($qode_options_eden['title_overlay_image'])){ 
		$title_overlay_image = $qode_options_eden['title_overlay_image']; 
	} 
}
...
if(isset($qode_options_eden['header_bottom_appearance']) && ($qode_options_eden['header_bottom_appearance'] == 'stick menu_bottom')){ 
	$menu_bottom = '46';
...
}else if($qode_options_eden['title_height'] != ''){
	$title_height = $qode_options_eden['title_height'];
}else {
	if (isset($qode_options_eden['center_logo_image']) && $qode_options_eden['center_logo_image'] == "yes") { 
		if($qode_options_eden['header_bottom_appearance'] == 'stick menu_bottom'){
...
if(get_post_meta($id, "qode_page-title-background-color", true) != ""){
 $title_background_color = get_post_meta($id, "qode_page-title-background-color", true);
	...
	$title_background_color = $qode_options_eden['title_background_color'];
}
...
if(get_post_meta($id, "qode_page_title_style", true) != ""){
	...
	if(isset($qode_options_eden['title_style'])) {
		$qode_page_title_style = $qode_options_eden['title_style'];
	} else {
		$qode_page_title_style = "standard";
	}
}
...
if(get_post_meta($id, "qode_animate-page-title", true) != ""){
	...
	if(isset($qode_options_eden['animate_title_area'])){ 
		$animate_title_area = $qode_options_eden['animate_title_area']; 
	}
}
...
}elseif(isset($qode_options_eden['enable_breadcrumbs'])){
	$enable_breadcrumbs = $qode_options_eden['enable_breadcrumbs'];
}
...
if(get_post_meta($id, "qode_page_title_font_size", true) != ""){
	...
	if(isset($qode_options_eden['predefined_title_sizes'])) {
		$page_title_fontsize = "title_size_" . $qode_options_eden['predefined_title_sizes'];
	}
}
$title_subtitle_padding ="";
if(get_post_meta($id, "qode_header_color_transparency_per_page", true) == ""){
	if($qode_options_eden['header_background_transparency_initial'] == "" || $qode_options_eden['header_background_transparency_initial'] == "1"){
...
if(get_post_meta($id, "qode_page_title_position", true) != ""){
	...
	$page_title_position = $qode_options_eden['page_title_position'];
}
if($page_title_position == "center"){
	$page_title_position_class = "position_center ";
}elseif($page_title_position == "left_image"){
	$page_title_position_class = "position_left_image ";
}elseif($page_title_position == "left_large"){
	$page_title_position_class = "position_left_large ";
}else {
	$page_title_position_class = "position_left ";
}
...
if(get_post_meta($id, "qode_title_text_shadow", true) != ""){
	...
	if(isset($qode_options_eden['title_text_shadow']) && ($qode_options_eden['title_text_shadow'] == "yes")){
		$title_text_shadow = ' title_text_shadow';
	}
}
$subtitle_color = "";
...
if (is_tag()) {
	...
	if($qode_options_eden['404_title'] != "") {
		$title = $qode_options_eden['404_title']; 
	} else { 
		$title = __('404', 'qode');
	}
	if($qode_options_eden['404_subtitle'] != "") {
		$title  .= $qode_options_eden['404_subtitle']; 
	} else { 
...
if(!get_post_meta($id, "qode_show-page-title", true)) { ?>
	<div class="title_outer <?php echo $animate_title_class.$title_text_shadow; if($responsive_title_image == 'yes' && $show_title_image == true){ echo ' with_image'; }?>" <?php echo 'data-height="'.$title_height.'"'; if($title_height != '' && $animate_title_area == 'area_top_bottom'){ echo 'style="opacity:0;height:' . $header_height_padding .'px;"'; } ?>>
		<div class="title <?php echo $page_title_fontsize . " " . $page_title_position_class; if($responsive_title_image == 'no' && $title_image != "" && ($fixed_title_image == "yes" || $fixed_title_image == "yes_zoom") && $show_title_image == true){ echo ' has_fixed_background '; if($fixed_title_image == "yes_zoom"){ echo 'zoom_out '; } } if($responsive_title_image == 'no' && $title_image != "" && $fixed_title_image == "no" && $show_title_image == true){ echo 'has_background'; }  ?>" style="<?php if($responsive_title_image == 'no' && $title_image != "" && $show_title_image == true){ if($title_image_width != ''){ echo 'background-size:'.$title_image_width.'px auto;'; } echo 'background-image:url('.$title_image.');';  } if($title_height != ''){ echo 'height:'.$title_height.'px;'; } if($title_background_color != ''){ echo 'background-color:'.$title_background_color.';'; } ?>">
					...
					<div class="container">
						<div class="container_inner clearfix">
								<div class="title_subtitle_holder" <?php if($responsive_title_image == 'yes' && $show_title_image == true){ echo $title_subtitle_padding; }?>>
									...
									<?php } ?>
									<?php if($page_title_position == "left_image"){ ?> <div class="title_thumb"><?php echo get_the_post_thumbnail($id, 'page_image'); ?></div><?php } ?>
										
										<?php if($page_title_position == "left_image"){ ?> <div class="title_thumb_text"><?php } ?>
										...
										<?php if (!is_singular('portfolio_page') && !is_single()) { ?>
										...
										<?php if($page_title_position == "left_image"){ ?> <div class="title_thumb_text_separator"></div><?php } ?>
										<?php if (is_singular('portfolio_page') || is_single()) { ?>
											<?php if(get_post_meta($id, "qode_page_subtitle", true) != ""){ ?>
												<span class="subtitle" <?php echo $subtitle_color; ?>><?php echo get_post_meta($id, "qode_page_subtitle", true); ?></span>
											<?php } ?>
												...
												<div class="breadcrumbs">
													<div class="breadcrumbs_inner">
														<div class="portfolio_social_holder top_share">
															...
															<?php } ?>
															<?php echo do_shortcode('[social_share_single]'); ?>
														</div>
													</div>
												</div>
											</div>
										<?php } else if (function_exists('qode_custom_breadcrumbs') && $enable_breadcrumbs == "yes") { ?>
											<div class="breadcrumb"> <?php qode_custom_breadcrumbs(); ?></div>
										<?php } ?>
										
										<?php if($page_title_position == "left_image"){ ?> </div><?php } ?>
