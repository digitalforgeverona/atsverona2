...
if (!class_exists('qode_type1_walker_nav_menu')) {
	...
	// add classes to ul sub-menus
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if($depth == 0){
				$out_div = '<div class="second bellow_header"><div class="inner">';
			}else{
				$out_div = '';
			}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ((($item->current && $depth == 0) ||  ($item->current_item_ancestor && $depth == 0)) && ($qode_options_eden['page_transitions'] == "0")):
			...
			if($item->hide == ""){
				...
				$angle_icon   = 'fa-angle-down';
				if($depth == 1) {
						$angle_icon = 'fa-angle-right';
				}

				$item_output .= '<i class="menu_icon fa '.$icon.'"></i>';
				$item_output .= '<span>'.apply_filters( 'the_title', $item->title, $item->ID ).'</span><span class="line"></span>';
		...
		// add main/sub classes to li's and links
		 function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($item->hide == ""){
				...
				if($item->nolink == ""){
					$item_output .= '<span class="mobile_arrow"><i class="fa fa-angle-right"></i><i class="fa fa-angle-down"></i></span></a>';
				}else{
					$item_output .= '<span class="mobile_arrow"><i class="fa fa-angle-right"></i><i class="fa fa-angle-down"></i></span></h3>';
				}
			}
			$item_output .= $args->after;
