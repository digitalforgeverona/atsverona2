...
if (!function_exists('action')) {
    function action($atts, $content = null) {
        ...
        if($text_left_padding != "") {
            $text_style = " style='padding-left: ".$text_left_padding."px;'";
        }
				...
				$html .= '<div class="call_to_action_icon">';
					$html .= '<div class="call_to_action_icon_inner">';
            			if($custom_icon != "") {
            				...
            				$html .= "<i class='fa ".$icon." ".$icon_size."' ".$icon_styles."></i>";
            			}
					$html .= '</div>';
				$html .= '</div>';
			$html .= '</div>';
        ...
        if($button_icon != ""){
            $add_icon .= "<i class='fa ".$button_icon."' style='".$add_icon_style."'></i>";
            $add_icon_class .= "qbutton_with_icon";
        }
	   
		$html .= '<div class="call_to_action_text" '.$text_style.'>'.do_shortcode($content).'</div>';
        ...
        if($show_button == 'yes') {
            ...
            $html .= '<a href="'.$button_link.'" class="qbutton '.$add_icon_class.'" target="'.$button_target.'" style="'.$button_styles.'">'.$add_icon.'<span>'.$button_text.'</span></a>';
...
function button($atts, $content = null) {
    ...
    } else if($type == "stript_button"){
        $add_icon .= '<i class="fa fa-angle-right"></i>';
    }
    ...
    if($type != "stript_button"){
        $html .=  '<a href="'.$link.'" target="'.$target.'" class="'.$button_classes.'" style="'.$button_styles.'">'.$add_icon.'<span>'.$text.'</span></a>';
    } else {
...
function counter($atts, $content = null) {
    ...
    if($digit_color_end != ""){
       ...
       $active_digit_color = "#222222";
    }

    $html .= '<div class="q_counter_holder '.$counter_holder_classes.'">';
    $html .= '<span class="counter '.$counter_classes.'" data-active_color="'.$active_digit_color.'" style="'.$counter_styles.'">'.$digit.'</span>';
...
function cover_boxes($atts, $content = null) {
    ...
    $html .= "<div class='box_content'><h4 ".$color1.">".$title1."</h4>";
    $html .= "<p ".$t_color1.">".$text1."</p>";
    $html .= "<a class='qbutton small stript_button' href='".$link1."' target='".$target1."'>".$link_label1."<i class='fa fa-angle-right'></i></a>";
    $html .= "</div></div>";
    $html .= "</li>";
    ...
    $html .= "<div class='box_content'><h4 ".$color2.">".$title2."</h4>";
    $html .= "<p ".$t_color2.">".$text2."</p>";
    $html .= "<a class='qbutton small stript_button' href='".$link2."' target='".$target2."'>".$link_label2."<i class='fa fa-angle-right'></i></a>";
    $html .= "</div></div>";
    $html .= "</li>";
    ...
    $html .= "<div class='box_content'><h4 ".$color3.">".$title3."</h4>";
    $html .= "<p ".$t_color3.">".$text3."</p>";
    $html .= "<a class='qbutton small stript_button' href='".$link3."' target='".$target3."'>".$link_label3."<i class='fa fa-angle-right'></i></a>";
    $html .= "</div></div>";
    $html .= "</li>";
    ...
    function icon_text($atts, $content = null) {
        ...
        if($box_type == "normal") {   
            ...
            if($link != ""){
                ...
                $html .= "<a class='icon_with_title_link' href='".$link."' target='".$target."' style='".$link_style."'>".$link_text."<i class='icon_with_title_link_mark fa fa-angle-right'></i></a>";
            }
...
/* Icon with text - Elegant shortcode */

if(!function_exists('icon_text_elegant')) {
    function icon_text_elegant($atts, $content = null) {
        $default_atts = array(
            "icon"                  => "",
            "icon_type"             => "", 
            "icon_size"             => "",
        ...
        if($icon_background_color != "") {
            ...
            $icon_arrow_style .= 'border-color: '.$icon_background_color.' transparent transparent transparent;';
        }
        ...
        $html_icon .= '<span class="fa-stack '.$icon_size.'" style="'.$icon_stack_style.'">';
        $html_icon .= '<i class="fa '.$icon.' fa-stack-1x" style="'.$style.'"></i>';
        $html_icon .= '<span class="icon_elegant_arrow" style="'.$icon_arrow_style.'"></span></span>'; 

        $html .= "<div class='q_icon_with_text_elegant ".$icon_classes."'>";

            //generate icon holder html part with icon
            $html .= '<div class="icon_elegant_holder">';
                $html .= $html_icon;
            $html .= '</div>';

            if($title != "" && $text != ""){
                //generate text html
                $html .= '<div class="icon_text_elegant_holder">';
                    $html .= '<'.$title_tag.' class="icon_elegant_title" style="'.$title_style.'">';
                    ...
                    if($separator == "yes"){
                        $html .= '<span class="icon_text_elegant_separator" style="'.$separator_style.'"></span>';
                    }
...
add_shortcode('icon_text_elegant', 'icon_text_elegant');
...
function icon_list_item($atts, $content = null) {
    ...
    $html .= '<i class="fa '.$icon.' '.$icon_classes.'" style="'.$icon_style.'"></i>';
...
if (!function_exists('latest_post')) {
    function latest_post($atts, $content = null) {
        ...
        $qode_like = "on";
        if (isset($qode_options_eden['qode_like'])) {
            $qode_like = $qode_options_eden['qode_like'];
        }
        ...
        $background_boxes_color = "";

        if($background_color != "" && $type == "boxes"){
            $background_boxes_color .= "background-color: ".$background_color.";";
        }
        ...
        while ($q->have_posts()) : $q->the_post();
            ...
            $html .= '<li class="clearfix" style="'.$background_boxes_color.'">';
            ...
            $html .= '<'.$title_tag.' class="latest_post_title "><a href="'.get_permalink().'" target="_self">' . get_the_title() . '</a></'.$title_tag.'>';
			...
			if($display_category == '1'){
				foreach ($cat as $categ) {
					$html .=' <a href="' . get_category_link($categ->term_id) . '">'.__("in ", "qode") . $categ->cat_name . '</a> ';
				}
			}
...
function line_graph($atts, $content = null) {
	...
	if(!empty($qode_options_eden['text_fontsize'])){
		$text_fontsize = $qode_options_eden['text_fontsize'];
	}else{
		$text_fontsize = 15;
	}
	if(!empty($qode_options_eden['text_color']) && $custom_color == ""){
		$text_color = $qode_options_eden['text_color'];
	} else if(empty($qode_options_eden['text_color']) && $custom_color != ""){
...
if (!function_exists('pie_chart')) {
    ...
    function pie_chart($atts, $content = null) {
        ...
        if($active_color != ""){
            $active_rgb_color = qode_hex2rgb($active_color);
            $active_bg_transparency = $percent/100;
            $active_bg_color .= "rgba(".$active_rgb_color[0].",".$active_rgb_color[1].",".$active_rgb_color[2].",".$active_bg_transparency.")";
        } else {
            if(!empty($qode_options_eden['first_color'])){
                $default_hex_color = $qode_options_eden['first_color'];
            } else {
                $default_hex_color = "#00c6ff";
            }

            $active_rgb_color = qode_hex2rgb($default_hex_color);
            $active_bg_transparency = $percent/100;
            $active_bg_color .= "rgba(".$active_rgb_color[0].",".$active_rgb_color[1].",".$active_rgb_color[2].",".$active_bg_transparency.")";
        }

        $html .= '<div class="q_pie_chart_holder"><div class="q_percentage" data-percent="' . $percent . '" data-linewidth="' . $line_width . '" data-active="' . $active_bg_color . '" data-noactive="' . $noactive_color . '">';
...
if (!function_exists('pie_chart_with_icon')) {
    ...
    function pie_chart_with_icon($atts, $content = null) {
        ...
        $html .= '<div class="q_pie_chart_with_icon_holder"><div class="q_percentage_with_icon" data-percent="' . $percent . '" data-linewidth="' . $line_width . '" data-active="' . $active_bg_color . '" data-noactive="' . $noactive_color . '">';
...
if (!function_exists('portfolio_list')) {
    ...
    function portfolio_list($atts, $content = null) {
        ...
        if (isset($qode_options_eden['portfolio_qode_like'])) {
            $portfolio_qode_like = $qode_options_eden['portfolio_qode_like'];
        }
        ...
        } elseif ($type == "standard_dark_style"){
            $_type_class = " standard dark_style";

        } elseif ($type == "hover_text") {
            $_type_class = " hover_text dark_style";
            $_portfolio_space_class = "portfolio_with_space";

        } elseif ($type == "hover_text_no_space"){
            $_type_class = " hover_text no_space dark_style";
                ...
                $html .= "<div class='image_holder'>";
                    $html .= "<a class='portfolio_link_for_touch' href='".get_permalink()."' target='_self'>";
                        $html .= "<span class='image'>";
        					$html .= get_the_post_thumbnail(get_the_ID(), 'portfolio_list');
                        $html .= "</span>";
                    $html .= "</a>";

                    if ($type == "standard" || $type == "standard_dark_style") {
						...
						$html .= "<span class='feature_holder'>";
                            ...
                            if ($lightbox == "yes") {
                                $html .= "<a class='lightbox' title='" . $title . "' href='" . $large_image . "' data-rel='prettyPhoto[" . $slug_list_ . "]'><i class='fa fa-search-plus fa-2x'></i></a>";
                            }
                            if($show_link_button == "yes"){
        ...
        $html .= "<li class='item'>";
            ...
            $html .= "<div class='image_holder'>";
                ...
                $html .= "<span class='feature_holder'>";
                    ...
                    $html .= '<span class="feature_holder_icons">';
                    if ($lightbox == "yes") {
                        $html .= "<a class='lightbox' title='" . $title . "' href='" . $large_image . "' data-rel='prettyPhoto[portfolio_slider]'><i class='fa fa-search-plus fa-2x'></i></a>";
                    }
                    if($show_link_button == "yes"){
                        $html .= "<a class='preview' href='" . get_permalink() . "'><i class='fa fa-link fa-2x'></i></a>";
                    }
    ...
    function service_table($atts, $content = null) {
        ...
        if($title_background_type == "background_color_type"){
            ...
            if($title_border_color != ""){
                $title_holder_style .= "border-top-color: ".$title_border_color.";";
                $service_table_style = $title_border_color;
            } else {
                if(!empty($qode_options_eden['first_color'])){
                    $service_table_style = $qode_options_eden['first_color'];
                } else {
        ...
        $title_classes .= $title_background_type . " " . $show_border;
        ...
        $html .= "<div class='service_table_holder'><ul class='service_table_inner' data-borderservice='".$service_table_style."'>";
...
if (!function_exists('progress_bar')) {
    ...
    function progress_bar($atts, $content = null) {
        ...
        if($percent != ""){
            $active_bg_transparency = $percent/100;
            $percentage_styles .= "opacity: ".$active_bg_transparency.";";
        }
...
function social_icons($atts, $content = null) {
    ...
    $html               = "";
    $fa_stack_styles    = "";
    $fa_stack_styles_hover = "";
    $icon_styles        = "";
    $icon_styles_hover = "";

    if($background_color != "" && $transparency == ""){
        $fa_stack_styles .= "background-color: {$background_color};";
    } else if ($background_color != "" && $transparency != ""){
        $bcolor = "";
        $bcolor = qode_hex2rgb($background_color);
        $fa_stack_styles .= "background-color: rgba(".$bcolor[0].",".$bcolor[1].",".$bcolor[2].",".$transparency.");";
    }

    if($background_color_hover != "" && $transparency_hover == ""){
        $fa_stack_styles_hover .= "background-color: {$background_color_hover};";
    } else if ($background_color_hover != "" && $transparency_hover != ""){
        $bcolor_hover = "";
        $bcolor_hover = qode_hex2rgb($background_color_hover);
        $fa_stack_styles_hover .= "background-color: rgba(".$bcolor_hover[0].",".$bcolor_hover[1].",".$bcolor_hover[2].",".$transparency_hover.");";
    }
    ...
    if($icon_color_hover != ""){
        $icon_styles_hover .= "color: ".$icon_color_hover.";";
    }
    ...
    if($link != ""){
        ...
        if($type == "normal_social"){
            ...
            $icon_class = str_replace('fa-','',$icon);

            $html .= "<i class='fa ".$icon." ".$size." simple_social' style='".$icon_styles."'></i>";

            $html .= "<i class='fa ".$icon." ".$size." simple_social social_share_hover ".$icon_class."' style='".$icon_styles_hover."'></i>";    
        } else {

            $icon_class = str_replace('fa-','',$icon);

            $html .= "<span class='fa-stack ".$size." ".$type."' style='".$fa_stack_styles."'>";
            $html .= "<i class='fa ".$icon."' style='".$icon_styles."'></i>";
            $html .= "</span>"; //close fa-stack

            $html .= "<span class='fa-stack ".$size." ".$type." social_share_hover ".$icon_class."' style='".$fa_stack_styles_hover."'>";
            $html .= "<i class='fa ".$icon."' style='".$icon_styles_hover."'></i>";
            $html .= "</span>"; //close fa-stack
        }
...
function social_share($atts, $content = null) {
    ...
    if(isset($qode_options_eden['twitter_via']) && !empty($qode_options_eden['twitter_via'])) {
		$twitter_via = " via " . $qode_options_eden['twitter_via'] . " ";
	} else {
		$twitter_via = 	"";
	}
	$image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full'); 
    $html = "";  
	if(isset($qode_options_eden['enable_social_share']) && $qode_options_eden['enable_social_share'] == "yes") { 
		$post_type = get_post_type();
		if(isset($qode_options_eden["post_types_names_$post_type"])) {
			if($qode_options_eden["post_types_names_$post_type"] == $post_type) {
			...
			$html .= '<div class="social_share_holder">';
				$html .= '<a href="javascript:void(0)" target="_self"><span class="social_share_icon"></span>';
					...
					if(isset($qode_options_eden['enable_facebook_share']) &&  $qode_options_eden['enable_facebook_share'] == "yes") { 
						$html .= '<li class="facebook_share">';
    						$html .= '<a href="#" onclick="window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . mysql_real_escape_string(get_the_title()) . '&amp;p[summary]=' . mysql_real_escape_string(get_the_excerpt()) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;&p[images][0]=';
        						...
        						if(!empty($qode_options_eden['facebook_icon'])) {
        							$html .= '<img src="' . $qode_options_eden["facebook_icon"] . '" alt="" />';
        						} else { 
					...
					if($qode_options_eden['enable_twitter_share'] == "yes") { 
						$html .= '<li class="twitter_share">';
                            $html .= '<a href="#" onclick="popUp=window.open(\'http://twitter.com/home?status=' . urlencode(the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
								if(!empty($qode_options_eden['twitter_icon'])) { 
									$html .= '<img src="' . $qode_options_eden["twitter_icon"] . '" alt="" />';
								 } else { 
					...
					if($qode_options_eden['enable_google_plus'] == "yes") { 
							$html .= '<li  class="google_share">';
									...
									if(!empty($qode_options_eden['google_plus_icon'])) { 
										$html .= '<img src="' . $qode_options_eden['google_plus_icon'] . '" alt="" />';
									} else { 
					...
					if(isset($qode_options_eden['enable_linkedin']) && $qode_options_eden['enable_linkedin'] == "yes") { 
						$html .= '<li  class="linkedin_share">';
                            $html .= '<a href="#" onclick="popUp=window.open(\'http://linkedin.com/shareArticle?mini=true&amp;url=' . urlencode(get_permalink()). '&amp;title=' . urlencode(get_the_title()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
								if(!empty($qode_options_eden['linkedin_icon'])) { 
									$html .= '<img src="' . $qode_options_eden['linkedin_icon'] . '" alt="" />';
								} else { 
					...
					if(isset($qode_options_eden['enable_tumblr']) && $qode_options_eden['enable_tumblr'] == "yes") { 
						$html .= '<li  class="tumblr_share">';
					       	$html .= '<a href="#" onclick="popUp=window.open(\'http://www.tumblr.com/share/link?url=' . urlencode(get_permalink()). '&amp;name=' . urlencode(get_the_title()) .'&amp;description='.urlencode(get_the_excerpt()) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
								if(!empty($qode_options_eden['tumblr_icon'])) { 
									$html .= '<img src="' . $qode_options_eden['tumblr_icon'] . '" alt="" />';
								} else { 
					...
					if(isset($qode_options_eden['enable_pinterest']) && $qode_options_eden['enable_pinterest'] == "yes") { 
						$html .= '<li  class="pinterest_share">';
							$html .= '<a href="#" onclick="popUp=window.open(\'http://pinterest.com/pin/create/button/?url=' . urlencode(get_permalink()). '&amp;description=' . mysql_real_escape_string(get_the_title()) .'&amp;media='.urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
								if(!empty($qode_options_eden['pinterest_icon'])) { 
									$html .= '<img src="' . $qode_options_eden['pinterest_icon'] . '" alt="" />';
								} else { 
					...
					if(isset($qode_options_eden['enable_vk']) && $qode_options_eden['enable_vk'] == "yes") { 
						$html .= '<li  class="vk_share">';
							...
							$html .= '<a href="#" onclick="popUp=window.open(\'http://vkontakte.ru/share.php?url=' . urlencode(get_permalink()). '&amp;title=' . urlencode(get_the_title()) .'&amp;description=' . urlencode(get_the_excerpt()) .'&amp;image='.urlencode($image[0]) . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false">';
								if(!empty($qode_options_eden['vk_icon'])) { 
									$html .= '<img src="' . $qode_options_eden['vk_icon'] . '" alt="" />';
								} else { 
...
/* Social Share shortcode single */

if (!function_exists('social_share_single')) {
function social_share_single($atts, $content = null) {
	global $qode_options_eden;
	if(isset($qode_options_eden['twitter_via']) && !empty($qode_options_eden['twitter_via'])) {
		$twitter_via = " via " . $qode_options_eden['twitter_via'] . " ";
	} else {
	...
	if(isset($qode_options_eden['enable_social_share']) && $qode_options_eden['enable_social_share'] == "yes") { 
		...
		if(isset($qode_options_eden["post_types_names_$post_type"])) {
			...
			$html .= '<div class="social_share_holder_single">';
					$html .= '<ul>';
					if(isset($qode_options_eden['enable_facebook_share']) &&  $qode_options_eden['enable_facebook_share'] == "yes") { 
						$html .= '<li class="facebook_share">';
    						$html .= '<a href="#" onclick="window.open(\'http://www.facebook.com/sharer.php?s=100&amp;p[title]=' . mysql_real_escape_string(get_the_title()) . '&amp;p[summary]=' . mysql_real_escape_string(get_the_excerpt()) . '&amp;p[url]=' . urlencode(get_permalink()) . '&amp;&p[images][0]=';
					...
					if($qode_options_eden['enable_twitter_share'] == "yes") { 
						$html .= '<li class="twitter_share">';
                            $html .= '<a href="#" onclick="popUp=window.open(\'http://twitter.com/home?status=' . urlencode(the_excerpt_max_charlength(mb_strlen(get_permalink())) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');popUp.focus();return false;">';
								if(!empty($qode_options_eden['twitter_icon'])) { 
									$html .= '<img src="' . $qode_options_eden["twitter_icon"] . '" alt="" />';
								 } else { 
									$html .= '<i class="fa fa-twitter"></i>';
...
add_shortcode('social_share_single', 'social_share_single');
    ...
    $html =  "<div class='q_team ".$type."'>";
		$html .=  "<div class='q_team_inner'>";
			...
			$html .=  "<div class='q_team_text'>";
                ...
                if($type == "normal"){
                    $social_type = "square_social";
                } else {
                    $social_type = "normal_social";
                }

                $html .=  "<div class='q_team_social_holder'>";
                    if($team_social_icon_1 != "") {
                        $html .=  do_shortcode('[social_icons icon="'. $team_social_icon_1 .'" type="'.$social_type.'" size="fa-2x" link="' . $team_social_icon_1_link . '" target="' . $team_social_icon_1_target . '"]');
                    }
                    if($team_social_icon_2 != "") {
                        $html .=  do_shortcode('[social_icons icon="'. $team_social_icon_2 .'" type="'.$social_type.'" size="fa-2x" link="' . $team_social_icon_2_link . '" target="' . $team_social_icon_2_target . '"]');
                    }
                    if($team_social_icon_3 != "") {
                        $html .=  do_shortcode('[social_icons icon="'. $team_social_icon_3 .'" type="'.$social_type.'" size="fa-2x" link="' . $team_social_icon_3_link . '" target="' . $team_social_icon_3_target . '"]');
                    }
                    if($team_social_icon_4 != "") {
                        $html .=  do_shortcode('[social_icons icon="'. $team_social_icon_4 .'" type="'.$social_type.'" size="fa-2x" link="' . $team_social_icon_4_link . '" target="' . $team_social_icon_4_target . '"]');
                    }
                $html .=  "</div>";
			$html .=  "</div>";
		$html .=  "</div>";
...
if (!function_exists('testimonials')) {
    ...
    function testimonials($atts, $content = null) {
		...
		if($border_color != ''){
			$testimonial_text_inner_styles .= "border: 1px solid ".$border_color.';';
		}

        if($company_color != ""){
            $testimonial_company_styles .= "color: ".$company_color.";";
        }
        				...
        				if($type == "standard") {
        				    ...
        				    $html .= '<div class="author_image_holder clearfix">';
                                ...
                                $html .= '<div class="testimonial_author">';
                					...
                					$html .= '<h5><span class="testimonial_name" style="'.$testimonial_name_styles.'">' . $author .'</span></h5>';

                					if($company_position != "") {
                						$html .= '<span class="company_position" style="'.$testimonial_company_styles.'">' . $company_position .'</span>';
                					}

                					if($website != "") {
                						$html .= '<span class="company_website" style="'.$testimonial_company_styles.'">' . $website .'</span>';
                					}
...
function qode_slider( $atts, $content = null ) {
	...
	if ($slider != "") {
		...
		if(isset($qode_options_eden['header_background_transparency_initial']) && $qode_options_eden['header_background_transparency_initial'] != "1" && $qode_options_eden['header_background_transparency_initial'] != ""){
		...
		if((get_post_meta($page_id, "qode_header_color_transparency_per_page", true) == "" || get_post_meta($page_id, "qode_header_color_transparency_per_page", true) == "1") && ($qode_options_eden['header_background_transparency_initial'] == "" || $qode_options_eden['header_background_transparency_initial'] == "1")){
			if (!empty($qode_options_eden['header_height'])) {
				$header_height = $qode_options_eden['header_height'];
			} else {
				$header_height = 85;
			}
			if($qode_options_eden['header_bottom_appearance'] == 'stick menu_bottom'){
				$menu_bottom = '46';
			...
			$header_top = 0;
			if(isset($qode_options_eden['header_top_area']) && $qode_options_eden['header_top_area'] == "yes"){
				$header_top = 34;	
			}
			$header_height_padding = $header_height + $menu_bottom + $header_top;
			if (isset($qode_options_eden['center_logo_image']) && $qode_options_eden['center_logo_image'] == "yes") { 
				if(isset($qode_options_eden['logo_image'])){
							$logo_width = 0;
							$logo_height = 0;
		...
		if($header_height_padding != 0){ 
			$navigation_margin_top = 'style="margin-top:'. ($header_height_padding/2 - 27).'px;"'; // 27 is half of navigation arrow
      ...
      $found_slides =  $wp_query->post_count;
        ...
        $title = get_the_title();
				...
				if(get_post_meta(get_the_ID(), "qode_slide-button-label2", true) != "" && get_post_meta(get_the_ID(), "qode_slide-button-link2", true) != ""){
					$html_text .= '<a style="border-color:#ffffff; color:#ffffff;" class="qbutton transparent_button" href="'.get_post_meta(get_the_ID(), "qode_slide-button-link2", true).'">'.get_post_meta(get_the_ID(), "qode_slide-button-label2", true).'</a>';
				}
				$html_text .= '</div>';
