...
function ajaxSetActiveState(me){
	...
	$j('header').removeClass('line_animation'); // for menu upperline animation on page transition
}
...
function slideOutOldPage(content, direction, direction2, animationTime, callbacks) { 
	...
	$j('html, body').animate({scrollTop: 0}, 400, function(){
		if($j('.title.position_left_image').length){
			$j('.title').addClass('hide_title_thumb');
		}
...
function onLinkClicked(me) {
	...
	if(!me.hasClass('current')){
		$j('header').addClass('line_animation'); // for menu upperline animation on page transition
