...
if (isset($qode_options_eden['hide_contact_form_website'])) $hide_contact_form_website = $qode_options_eden['hide_contact_form_website'];
...
<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
		...
		<?php if($qode_options_eden['enable_google_map'] == "yes"){ ?>
		...
		<div class="container_inner<?php echo $container_class; ?> clearfix">
				<div class="contact_detail">
					...
					<?php if($qode_options_eden['enable_contact_form'] == "yes"){ ?>
					<div class="two_columns_66_33 clearfix grid2">
						<div class="column1">
							<div class="column_inner">
								<div class="contact_form">
									<h4><i class="fa fa-envelope-o"></i><?php if($qode_options_eden['contact_heading_above'] != "") { echo $qode_options_eden['contact_heading_above'];  } else { ?><?php _e('Contact Us', 'qode'); ?><?php } ?></h4>
									<form id="contact-form" method="post" action="">
										...
										<?php }?>
										<textarea name="message" id="message" rows="10" placeholder="<?php _e('Your message here', 'qode'); ?>"></textarea>
										
										<?php
										if($qode_options_eden['use_recaptcha'] == "yes") :
											require_once('includes/recaptchalib.php');
											if($qode_options_eden['recaptcha_public_key']) {
												$publickey = $qode_options_eden['recaptcha_public_key'];
											} else {
												$publickey = "6Ld5VOASAAAAABUGCt9ZaNuw3IF-BjUFLujP6C8L";
											}
											if($qode_options_eden['recaptcha_private_key']) {
												$privatekey = $qode_options_eden['recaptcha_private_key'];
											} else {
												$privatekey = "6Ld5VOASAAAAAKQdKVcxZ321VM6lkhBsoT6lXe9Z";
											}

											if($qode_options_eden['page_transitions'] != ""){ ?>
										...
										<span class="submit_button_contact">
											<input class="qbutton small" type="submit" value="<?php _e('Send Message', 'qode'); ?>" />
										</span>
									</form>	
								</div>
							</div>
